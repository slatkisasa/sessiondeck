...
SessionDeck is a small Windows 11 x64 WinForms application for switching between Battle.net accounts whose authenticated sessions are already retained by the Battle.net launcher. It is currently intended for personal use, but repository changes should remain safe and understandable for other users.
...
- Run non-destructive tests with `./.dotnet-sdk/dotnet.exe run --project ./SessionDeck.csproj -- --self-test` when the repository-local SDK exists, or use `dotnet run` with the same arguments otherwise.
